...
async function generateAndPlayAudio(text: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    exec(command, (error, stdout, stderr) => {
      ...
      if (
        stdout.includes("Error generating audio") ||
      ...
      try {
        ...
        audioPlayer.play(outputPath, (err: Error | null) => {
          if (err) {
            ...
            if (playCmd) {
              ...
              exec(playCmd, (playError, playStdout, playStderr) => {
                if (playError) {
                  console.error(
                    "Error playing audio with fallback command:",
                    playError
                  );
                  ...
                  console.log(
                    "Audio played successfully with fallback command"
                  );
              ...
              console.log(
                "No fallback audio playback available, continuing anyway"
              );
